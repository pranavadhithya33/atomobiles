...
export async function scrapeFlipkartProduct(url, category) {
  ...
  try {
    // Force mobile domain for better scraper compatibility
    const fetchUrl = url.includes('m.flipkart.com') ? url : url.replace('www.flipkart.com', 'm.flipkart.com');
    ...
    if (res.ok) {
      ...
      const proxyUrl1 = `https://api.allorigins.win/raw?url=${encodeURIComponent(fetchUrl)}`;
      ...
      if (proxyRes1.ok) {
        ...
        const proxyUrl2 = `https://api.corsproxy.io/?url=${encodeURIComponent(fetchUrl)}`;
        try {
    ...
    if (!title || title.toLowerCase().includes('flipkart')) {
      const match = url.match(/flipkart\.com\/([^\/]+)\//);
      if (match?.[1]) title = match[1].replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
    }
    if (!title) title = 'Flipkart Product';

    // ── IMAGES ─────────────────────────────────────────────────────────
    const images = [];
    const addImage = (src) => {
      if (!src || typeof src !== 'string' || src.includes('gif') || src.includes('base64')) return;
      if (src.startsWith('//')) src = 'https:' + src;
      // Convert thumbnail URLs to high-res (800x800, quality 90)
      let hiRes = src.replace(/\/\d+\/\d+\//, '/800/800/').replace(/q=\d+/, 'q=90');
      if (!images.includes(hiRes)) images.push(hiRes);
    };

    // Main image selectors
    const mainImg = $('img.DByo73, img._396cs4, img._2r_T1_, img._53u0Z-, img._1XjE3w').first().attr('src');
    if (mainImg) addImage(mainImg);

    // Gallery and list images
    $('li._208S6Y img, img.q6DClP, img._1B0S_8, img.j-m87M, ._2AmZf7 img').each((_, el) => {
      const src = $(el).attr('src') || $(el).attr('data-src');
      if (src) addImage(src);
    });

    // Fallback 1: search for all product-like images in DOM
    $('img').each((_, el) => {
      const src = $(el).attr('src') || $(el).attr('data-src');
    ...
    // Fallback 2: Regex scan the entire HTML for Flipkart image patterns
    // This catches images hidden in JavaScript or data attributes
    const imgRegex = /https:\/\/rukminim[0-9]\.flixcart\.com\/image\/\d+\/\d+\/[a-zA-Z0-9\-\/]+/g;
    ...
    // Regex fallback for price (searching for ₹ followed by numbers)
    if (!flipkartPrice || flipkartPrice < 100) {
      const priceMatches = html.match(/₹([0-9,]+)/g);
      if (priceMatches) {
        // Filter out very small numbers (likely counts or discounts)
