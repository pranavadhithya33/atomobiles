...
export async function scrapeFlipkartProduct(url, category) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      // Fall back to allorigins proxy if direct fetch fails
      console.log(`Direct Flipkart fetch failed (${res.status}), trying proxy...`);
      const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(url)}`;
      const proxyRes = await fetch(proxyUrl, { cache: 'no-store' });
      if (!proxyRes.ok) throw new Error(`Flipkart unavailable (${res.status}). Try again in a moment.`);
    ...
    // Fallback: search for all product-like images
    if (images.length === 0) {
      $('img').each((_, el) => {
